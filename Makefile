...
tidy:
	@go mod tidy
	@for a in $(ADAPTERS); do cd adapters/$$a && go mod tidy && cd ../..; done
...
lint:
	@golangci-lint run
	@for a in $(ADAPTERS); do cd adapters/$$a && golangci-lint run && cd ../..; done
...
.PHONY: check-release
check-release: sync tidy fix-replace lint test
